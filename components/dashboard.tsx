    ...
    <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
      <section className="card">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <div className="rounded bg-slate-800 p-2">Completed Box: <b>{summary.completedBoxes}</b></div>
        </div>
        ...
        <ul className="max-h-[50vh] space-y-1 overflow-auto text-sm">
          {latest.map((event) => (
            <li key={event.id} className="rounded bg-slate-800 p-2">
              {new Date(event.timestamp).toLocaleTimeString("tr-TR")} | Box {event.boxNo} |{" "}
              <span className={event.status === "OK" ? "text-ok" : event.status === "NOK" ? "text-nok" : "text-duplicate"}>
                {event.status}
              </span>{" "}
              | {event.code}
            </li>
